...
def train(
    ...
    if not binary:
        ...
        num_labels = 2
    # If retraining the model from scratch use the next line
    model = LongformerForSequenceClassification.from_pretrained('allenai/longformer-base-4096', num_labels = num_labels)
    # If reusing pretrained weights uncomment the next three lines and comment the above line.
    ...
    for epoch in range(1, epochs):
        ...
        if save:
            ...
            torch.save(model, f"longformer__{nodes}__{label}__e{epoch}.pickle")

    logger.success("Done!")

train(binary=False, root=False, save=True)
